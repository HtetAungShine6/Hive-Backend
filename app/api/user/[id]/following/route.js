...
export async function GET(req, { params }) {
  ...
  try {
    ...
    const following = await Relationship.find({ follower: id }).populate(
      'following',
    ...
    const result = following
      ...
      .map((relationship) => ({
        ...
        about: relationship.following.about,
      }))

    return NextResponse.json(
      { success: true, following: result },
      { status: 200 }
    )
  } catch (error) {
    console.error('Error fetching following:', error.message, error.stack)
    return NextResponse.json(
      { success: false, message: 'Error fetching following' },
      { status: 500 }
    )
  }
}
